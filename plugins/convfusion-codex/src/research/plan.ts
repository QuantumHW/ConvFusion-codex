...
/** 可交接的状态。 */
const HANDOFF_STATUSES: readonly PlanStatus[] = ['ready', 'refined']
...
export interface PlanHandoffError {
  error: string
  /** 可选的候选项，便于调用方提示用户。 */
  available?: Array<{ id: string; name: string; status: PlanStatus; relPath: string }>
}
 ...
 */
export function loadPlanHandoff(
  workspace: string,
  planId?: string,
...
function summarize(d: PlanDocument): { id: string; name: string; status: PlanStatus; relPath: string } {
 ...
 */
export function buildHandoffText(doc: PlanDocument): string {
  ...
  if (criteria.trim() || evidence.trim()) {
    ...
    if (objective.trim()) lines.push('', `Objective: ${objective.split('\n')[0]}`)
    if (criteria.trim()) lines.push('', 'Completion criteria:', '', criteria)
    if (evidence.trim()) lines.push('', 'Evidence to produce:', '', evidence)
  }
  ...
  lines.push(
    ...
    '---',
    '',
    'When finished, summarise: what was done, what was produced (paths), what was verified,',
...
export function handoffPlanToAgent(agent: FollowupCapableAgent, text: string): void {
  agent.followup(createPlanMessage(text))
}

/** 供插件入口复用：解析会话 workspace 后的 Plan 交接。 */
export function handoffPlan(workspace: string, agent: FollowupCapableAgent, planId?: string): { ok: true; planId: string } | PlanHandoffError {
  const handoff = loadPlanHandoff(workspace, planId)
  if ('error' in handoff) return handoff
  // §19：交接是"进入执行"的必经点 —— 先把当前内容快照，再交给 Harness。
  // 之后 Agent 用原生工具接着改 `plans/*.md`，历史已经保住了。
  const doc = readPlan(workspace, handoff.planId)
  if (doc) archivePlanVersion(workspace, doc, 'handoff')
