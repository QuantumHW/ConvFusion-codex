...
export function renderFrontmatter(fields: Record<string, string | number | undefined>): string {
  const lines = ['---']
 ...
 */
function fenceTracker(): { inFence: () => boolean; feed: (line: string) => void } {
...
/** 渲染 `# 标题` + `##` 章节。 */
export function renderSections(title: string, sections: Array<{ title: string; body: string }>): string {
  const parts = [`# ${title}`, '']
  for (const s of sections) parts.push(`## ${s.title}`, '', s.body.trim(), '')
  return parts.join('\n').replace(/\n{3,}/g, '\n\n').trim() + '\n'
}

/** 按标题取章节（宽松匹配：大小写与标点不敏感）。 */
export function findSection(sections: Array<{ title: string; body: string }>, ...names: string[]): string {
...
/** 渲染 id 列表。 */
export function renderIdList(ids: readonly string[]): string {
  return ids.length === 0 ? '(none)' : ids.join(', ')
}

/** 剥离归档/时间戳注释（历史快照的幂等比较用）。 */
const ARCHIVE_HEADER_RE = /^<!--\s*(?:archived|snapshot):[\s\S]*?-->\r?\n/
export function stripArchiveHeader(source: string): string {
...
export function writeSnapshotIfChanged(
  ...
  if (exists(file)) {
    ...
    if (existing !== undefined && stripArchiveHeader(existing).trim() === content.trim()) return false
  }
...
export function parseListItems(section: string): string[] {
  ...
  for (const raw of section.split(/\r?\n/)) {
    ...
    if (!text) continue
    if (startsItem || items.length === 0) items.push(text)
    else items[items.length - 1] = `${items[items.length - 1]} ${text}`
  }
  return items
}
 ...
 */
export function deriveShortName(text: string, max = 80): string {
  const clean = text.replace(/\s+/g, ' ').trim()
  if (!clean) return ''
  if (clean.length <= max) return clean
  // 先按句末标点取整句（中英文都要认）
  const firstSentence = clean.split(/[。；;]|(?<=[.。!！?？])\s/)[0]?.trim() ?? clean
  if (firstSentence.length > 0 && firstSentence.length <= max) return firstSentence
