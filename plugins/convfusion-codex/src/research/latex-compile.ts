...
export function findTectonic(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  for (const c of TECTONIC_CANDIDATES) {
    if (!c.includes('/')) {
      ...
      if (which.status === 0 && out) return out
      continue
    }
 ...
 */
export function parseCompileErrors(raw: string): CompileError[] {
  if (!raw || !raw.trim()) return []
  const lines = raw.split(/\r?\n/)
  const found: CompileError[] = []
  const seen = new Set<string>()

  const push = (line: number, message: string, severity: CompileError['severity']): void => {
    const msg = message.trim()
    if (!msg) return
    const lower = msg.toLowerCase()
    // 排版警告不当作错误（旧版同样跳过）
    if (lower.includes('underfull') || lower.includes('overfull')) return
 ...
 */
export function compileLatex(texPath: string, opts: CompileOptions = {}): CompileResult {
 ...
 */
export function escapePercentOutsideMath(latex: string): string {
  ...
  for (let i = 0; i < regions.length; i++) out = out.split(`\u0000PMATH${i}\u0000`).join(regions[i])
  return out
}
 ...
 */
function fixSingleLine(line: string, errorMessage: string): string {
  ...
  if (lower.includes('invalid in math mode')) {
    out = out.replace(/\$([^$]*?)\\cite\{([^}]+)\}([^$]*)\$/g, (_m, pre: string, key: string, post: string) => {
 ...
 */
export function repairLatex(latex: string, errors: readonly CompileError[]): RepairResult {
  let out = escapePercentOutsideMath(latex)
  const remaining: CompileError[] = []
  let fixedCount = 0
...
export function scanUnsafeTokens(latex: string): GrammarScan {
  const unknownCommands: string[] = []
  const unknownEnvs: string[] = []
  const seenCmd = new Set<string>()
  const seenEnv = new Set<string>()

  for (const m of latex.matchAll(/\\([a-zA-Z]+)/g)) {
    const cmd = m[1]
    if (KNOWN_COMMANDS.has(cmd) || seenCmd.has(cmd)) continue
  ...
  for (const m of latex.matchAll(/\\begin\{([^}]+)\}/g)) {
    const env = m[1]
    if (KNOWN_ENVS.has(env) || seenEnv.has(env)) continue
    seenEnv.add(env)
