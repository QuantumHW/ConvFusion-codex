...
/** 生成一个带编号的 id，如 `E007`。 */
export function makeResearchId(kind: ResearchAssetKind, n: number): string {
  return `${ID_PREFIX[kind]}${String(n).padStart(3, '0')}`
}

/** 解析 id → `{ kind, n }`；非法返回 `undefined`。 */
export function parseResearchId(id: string): { kind: ResearchAssetKind; n: number } | undefined {
  const m = id.trim().match(/^([ECD P])(\d{1,4})$/i)
  if (!m) return undefined
  const prefix = m[1].toUpperCase()
  const entry = (Object.entries(ID_PREFIX) as Array<[ResearchAssetKind, string]>).find(([, p]) => p === prefix)
...
export const MATURITY_LEVELS: readonly MaturityLevel[] = ['Unknown', 'Weak', 'Emerging', 'Strong', 'Established']

/** 成熟度维度（§35 的六个）。 */
export type MaturityDimension = 'Problem' | 'Knowledge' | 'Innovation' | 'Method' | 'Experiment' | 'Evidence'

export const MATURITY_DIMENSIONS: readonly MaturityDimension[] = [
  'Problem',
  'Knowledge',
  'Innovation',
  'Method',
