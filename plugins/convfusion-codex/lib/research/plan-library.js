...
import { PLANS_DIR, PLAN_HISTORY_DIR, canTransition, filterPlans, isLegacyStylePlanId, listPlanDocuments, parsePlanDocument, parsePlanFrontmatter, readPlan, serializePlanDocument, toPlanId, transitionError, } from './plans.js';
/** 某个 Plan 的历史快照目录（绝对路径）。 */
function planHistoryDir(workspace, id) {
    return join(workspace, PLAN_HISTORY_DIR, id);
}
/** 构建 Library 视图（纯投影）。 */
export function loadPlanLibrary(workspace) {
    ...
    for (const s of ['draft', 'reviewed', 'ready', 'executing', 'completed', 'refined', 'archived']) {
...
export function discoverPlans(workspace, query = {}) {
    const view = loadPlanLibrary(workspace);
    const docs = filterPlans(view.entries.map((e) => e.document), query);
    const byId = new Map(view.entries.map((e) => [e.document.id, e]));
    return docs.map((d) => byId.get(d.id));
}
export function isPlanWriteError(v) {
    return typeof v === 'object' && v !== null && 'error' in v;
}
function ensurePlansDir(workspace) {
...
export function planSectionsTemplate(input) {
    ...
    if (input.questions)
        sections.push({ title: 'Research Questions', body: input.questions });
    sections.push({
    ...
    if (input.expectedOutputs)
        sections.push({ title: 'Expected Outputs', body: input.expectedOutputs });
    if (input.constraints)
        sections.push({ title: 'Constraints', body: input.constraints });
    sections.push({
...
export function createPlan(workspace, input) {
    ...
    if (!name)
        return { error: 'Plan 名称不能为空。' };
    const id = toPlanId(input.id ?? name);
    if (!id)
        return { error: `无法从名称生成合法 id：${name}` };
    if (isLegacyStylePlanId(id)) {
        return {
            error: `id \`${id}\` 是旧 Step 风格命名（§27 禁止）。请用能力/任务命名，` +
    ...
    if (existsSync(file))
        return { error: `Plan \`${id}\` 已存在。` };
    const now = new Date().toISOString();
    const sections = input.sections ?? planSectionsTemplate(input);
    writeFileSync(file, serializePlanDocument({
        name,
...
export function savePlanBody(workspace, id, body) {
    const doc = readPlan(workspace, id);
    if (!doc)
        return { error: `找不到 Plan \`${id}\`。` };
    writeFileSync(doc.path, body, 'utf8');
    return parsePlanDocument(doc.path, doc.relPath) ?? { error: '保存后解析失败。' };
}
/** 更新元数据（名称 / 类型 / 版本 / 策略 / 关联），正文保持不变。 */
export function updatePlanMeta(workspace, id, patch) {
    const doc = readPlan(workspace, id);
    if (!doc)
        return { error: `找不到 Plan \`${id}\`。` };
    writeFileSync(doc.path, serializePlanDocument({
...
export function updatePlanSection(workspace, id, title, body) {
    const doc = readPlan(workspace, id);
    if (!doc)
        return { error: `找不到 Plan \`${id}\`。` };
...
function writeSections(workspace, doc, sections) {
    writeFileSync(doc.path, serializePlanDocument({
        name: doc.name,
        type: doc.type,
        status: doc.status,
...
/** 删除一个 Plan（同时清理其版本快照）。 */
export function deletePlan(workspace, id) {
    const doc = readPlan(workspace, id);
    if (!doc)
        return { error: `找不到 Plan \`${id}\`。` };
    try {
 ...
 */
export function setPlanStatus(workspace, id, status) {
    const doc = readPlan(workspace, id);
    if (!doc)
        return { error: `找不到 Plan \`${id}\`。` };
    if (!canTransition(doc.status, status))
        return { error: transitionError(doc.status, status) };
    writeFileSync(doc.path, serializePlanDocument({
        name: doc.name,
        type: doc.type,
        status,
...
/** §32-E：用户已检查 → `reviewed`。 */
export function reviewPlan(workspace, id) {
    return setPlanStatus(workspace, id, 'reviewed');
}
/** §32-E：用户明确允许执行 → `ready`。 */
export function approvePlan(workspace, id) {
    return setPlanStatus(workspace, id, 'ready');
}
/** 标记为执行中（交接给 Harness 后调用）。 */
export function markPlanExecuting(workspace, id) {
    return setPlanStatus(workspace, id, 'executing');
}
/** 标记完成。 */
export function completePlan(workspace, id) {
    return setPlanStatus(workspace, id, 'completed');
}
/** 归档（不再作为当前任务）。 */
export function archivePlan(workspace, id) {
 ...
 * ════════════════════════════════════════════════════════════════════════ */
/** 递增语义版本：`1.0` → `1.1` → `2.0`。 */
export function bumpPlanVersion(version, kind = 'minor') {
...
/** 剥离归档注释，用于幂等比较。 */
export function stripPlanArchiveHeader(source) {
 ...
 */
export function archivePlanVersion(workspace, doc, note) {
    ...
    if (last) {
        ...
        if (previous !== undefined && stripPlanArchiveHeader(previous).trim() === current.trim())
            return undefined;
    }
    const seq = String(existing.length + 1).padStart(3, '0');
    const header = `<!-- archived: ${new Date().toISOString()} — plan_version: ${doc.version}` +
        `${note ? ` — ${note}` : ''} -->\n`;
    mkdirSync(dir, { recursive: true });
 ...
 */
export function syncPlanHistory(workspace) {
    const created = [];
...
export function syncPlanHistoryDetailed(workspace) {
    const out = [];
    for (const doc of listPlanDocuments(workspace)) {
        const snap = archivePlanVersion(workspace, doc);
        if (snap)
            out.push({ id: doc.id, snapshot: snap });
    }
    return out;
}
/** 读一个 Plan 的历史版本（`version` 是快照序号，如 `001`）。 */
export function readPlanVersion(workspace, id, version) {
...
export function listPlanVersions(workspace, id) {
    let files;
    try {
 ...
 */
export function revisePlan(workspace, id, input) {
...
export function buildPlanGenerationText(workspace, request, skill) {
    ...
    lines.push('', '---', '', `After writing the file, tell the user it is at \`${suggestedPath}\` and that they can review`, 'and edit it before approving execution.');
    void workspace;
    return { text: lines.join('\n'), suggestedId, suggestedPath };
}
//# sourceMappingURL=plan-library.js.map