 ...
 */
export function isResearchWorkspace(dir) {
    try {
...
export function resolveWorkspace(cwd, configured) {
 ...
 */
export function researchWorkspaceOf(sessionWorkspace) {
    const base = resolve(sessionWorkspace);
    const nested = join(base, RESEARCH_ROOT_DIR);
    if (isResearchWorkspace(nested))
        return nested;
    if (isResearchWorkspace(base))
        return base;
    return nested;
}
/* ════════════════════════════════════════════════════════════════════════
 ...
 */
export function loadPaper(workspace, excerptChars = 4000, paperId = DEFAULT_PAPER_ID) {
