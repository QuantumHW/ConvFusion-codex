...
import { EVIDENCE_DIR, EVIDENCE_SOURCES, EVIDENCE_STATUSES, RESEARCH_DIR, makeResearchId, parseResearchId, } from './research-data.js';
...
function normalizeSource(raw) {
    ...
    return EVIDENCE_SOURCES.includes(v) ? v : 'observation';
}
function normalizeStatus(raw) {
    const v = (raw ?? '').trim().toLowerCase();
    return EVIDENCE_STATUSES.includes(v) ? v : 'unverified';
}
...
export function parseEvidenceDocument(absPath, relPath) {
    let source;
    try {
    ...
    return {
        ...
        provenance: {
            ...
            ...(fm.source && !EVIDENCE_SOURCES.includes(fm.source) ? { source: fm.source } : {}),
...
export function listEvidence(workspace) {
...
/** 读一条 Evidence。 */
export function readEvidence(workspace, id) {
    const norm = id.trim().toUpperCase();
    return listEvidence(workspace).find((e) => e.id === norm);
}
/** 下一个可用 Evidence id（§37 的连续编号）。 */
export function nextEvidenceId(workspace) {
...
export function isEvidenceWriteError(v) {
...
export function evidenceSectionsTemplate(input) {
    const sections = [];
    sections.push({
        title: 'Claim',
    ...
    sections.push({
        ...
        body: input.rawArtifacts?.length
 ...
 */
export function createEvidence(workspace, input) {
    const name = input.name.trim();
    if (!name)
        return { error: 'Evidence 名称不能为空。' };
    ...
    if (existsSync(file))
        return { error: `Evidence \`${id}\` 已存在。` };
    const now = new Date().toISOString();
    const status = input.status ?? 'unverified';
    writeFileSync(file, serializeEvidenceDocument({
        id,
        name,
...
/** 记录一条历史快照（不删除性覆盖的实现）。 */
function snapshot(workspace, doc, note) {
...
/** 列出某条 Evidence 的历史快照。 */
export function listEvidenceHistory(workspace, id) {
...
/** 读取某个历史快照。 */
export function readEvidenceSnapshot(workspace, relPath) {
    if (!relPath.startsWith(EVIDENCE_HISTORY) || relPath.includes('..'))
        return undefined;
    try {
...
/** 整体保存正文（编辑器路径）。 */
export function saveEvidenceBody(workspace, id, body) {
    const doc = readEvidence(workspace, id);
    if (!doc)
        return { error: `找不到 Evidence \`${id}\`。` };
...
/** 替换单个章节。 */
export function updateEvidenceSection(workspace, id, title, body) {
    const doc = readEvidence(workspace, id);
    if (!doc)
        return { error: `找不到 Evidence \`${id}\`。` };
...
function writeEvidence(workspace, doc, patch) {
    writeFileSync(doc.path, serializeEvidenceDocument({
        id: doc.id,
        name: doc.name,
        type: doc.type,
...
export function setEvidenceStatus(workspace, id, status) {
    const doc = readEvidence(workspace, id);
    if (!doc)
        return { error: `找不到 Evidence \`${id}\`。` };
 ...
 */
export function supersedeEvidence(workspace, oldId, newId, note) {
    const oldDoc = readEvidence(workspace, oldId);
    if (!oldDoc)
        return { error: `找不到旧 Evidence \`${oldId}\`。` };
    const newDoc = readEvidence(workspace, newId);
    if (!newDoc)
        return { error: `找不到新 Evidence \`${newId}\`。` };
    ...
    const oldRes = writeEvidence(workspace, oldDoc, {
        ...
        sections: oldDoc.sections.some((s) => s.title === 'Validation')
            ? oldDoc.sections.map((s) => s.title === 'Validation'
 ...
 */
export function deleteEvidence(workspace, id, opts = {}) {
    const doc = readEvidence(workspace, id);
    if (!doc)
        return { error: `找不到 Evidence \`${id}\`。` };
...
export function contradictionsForClaim(workspace, claimId) {
    const norm = claimId.trim().toUpperCase();
