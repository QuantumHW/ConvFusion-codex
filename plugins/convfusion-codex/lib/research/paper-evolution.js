...
import { PAPER_FILES, PAPER_MATURITY_DIMENSIONS, } from './paper-data.js';
import { archivePaperVersion, buildEvidenceMap, bumpPaperVersion, listEvolutionEvents, paperDir, paperRelDir, readClaimMap, readPaper, recordEvolutionEvent, saveManuscript, } from './paper.js';
 ...
 * ════════════════════════════════════════════════════════════════════════ */
function proposalsDir(workspace, paperId) {
...
/** 解析一个提案文件。 */
export function parseProposal(source, fallbackId) {
    const fm = parseFrontmatter(source);
    const sections = parseSections(stripFrontmatter(source)).sections;
    const id = fm.id || fallbackId;
    if (!id)
        return null;
    const list = (title) => (findSection(sections, title).match(/^[-*]\s*(.+)$/gm) ?? []).map((l) => l.replace(/^[-*]\s*/, '').trim());
    return {
        id,
...
/** 列出全部提案。 */
export function listProposals(workspace, paperId) {
...
/** 读一个提案。 */
export function readProposal(workspace, paperId, proposalId) {
    return listProposals(workspace, paperId).find((p) => p.id === proposalId);
}
/** 下一个提案编号。 */
export function nextProposalId(existing) {
    const max = existing.reduce((n, p) => Math.max(n, Number(p.id.replace(/^RP?/, '')) || 0), 0);
    return `R${String(max + 1).padStart(3, '0')}`;
}
function writeProposal(workspace, paperId, p) {
 ...
 * 正文不变，等用户处置。
 */
export function proposeRevision(workspace, paperId, input) {
    const paper = readPaper(workspace, paperId);
    if (!paper)
        return { error: `找不到 Paper \`${paperId}\`。` };
    if (!input.reason.trim())
        return { error: '提案必须说明理由（§11 Reason）。' };
 ...
 */
export function analyzeEvidenceImpact(workspace, paperId, evidenceId) {
    ...
    return {
        ...
        claims: touched,
        sections: [...sections],
        recommendation: touched.length === 0
 ...
 */
export function applyRevision(workspace, paperId, proposalId, options = {
    action: 'accepted',
}) {
    const paper = readPaper(workspace, paperId);
    if (!paper)
    ...
    if (!proposal)
        return { error: `找不到提案 \`${proposalId}\`。` };
    if (proposal.status !== 'proposed') {
        return { error: `提案 \`${proposalId}\` 已处置（${proposal.status}），不能重复应用。` };
    }
    const fromVersion = paper.metadata.version;
    const finalText = options.finalText ?? (proposal.proposedChanges.trim() ? wrapProposalText(paper.manuscript, proposal) : paper.manuscript);
    ...
    recordEvolutionEvent(workspace, paperId, {
        fromVersion,
        toVersion,
        trigger: options.trigger ?? proposal.trigger ?? 'user-revision-request',
 ...
 */
function wrapProposalText(manuscript, proposal) {
...
function writeMetadataVersion(workspace, paperId, version) {
...
/** 拒绝一次提案（§11 reject）。 */
export function rejectRevision(workspace, paperId, proposalId, note) {
    const paper = readPaper(workspace, paperId);
    if (!paper)
        return { error: `找不到 Paper \`${paperId}\`。` };
    const proposal = readProposal(workspace, paperId, proposalId);
    if (!proposal)
        return { error: `找不到提案 \`${proposalId}\`。` };
    if (proposal.status !== 'proposed')
        return { error: `提案 \`${proposalId}\` 已处置。` };
    const resolved = {
        ...proposal,
        status: 'rejected',
        resolved: {
...
function maturityPath(workspace, paperId) {
    return join(paperDir(workspace, paperId), MATURITY_FILE);
}
/** 读成熟度（不存在 → 全部 Unknown 并给出原因）。 */
export function readPaperMaturity(workspace, paperId) {
    let source = null;
    try {
...
/** 写成熟度。 */
export function writePaperMaturity(workspace, paperId, maturity) {
    const fields = {};
    for (const d of PAPER_MATURITY_DIMENSIONS) {
 ...
 */
export function suggestPaperMaturity(workspace, paperId) {
    const paper = readPaper(workspace, paperId);
    ...
    const hasSection = (re) => (paper?.sections ?? []).some((s) => re.test(s.title) && s.body.replace(/<!--[\s\S]*?-->/g, '').trim().length > 60);
    const level = (n) => ['Unknown', 'Weak', 'Emerging', 'Strong', 'Established'][Math.min(4, n)];
    const out = {};
...
/** 供状态摘要使用的成熟度总览。 */
export function maturityOverview(maturity) {
    const established = [];
    const emerging = [];
    const missing = [];
    for (const d of PAPER_MATURITY_DIMENSIONS) {
        const l = maturity[d].status;
        if (l === 'Strong' || l === 'Established')
            established.push(d);
        else if (l === 'Unknown')
            missing.push(d);
        else
...
/** 汇总 Paper 状态（纯只读）。 */
export function paperStatusSummary(workspace, paperId) {
    const paper = readPaper(workspace, paperId);
    if (!paper)
        return undefined;
    const claims = readClaimMap(workspace, paperId);
...
/** 清理一个提案（用于测试/误建）。 */
export function deleteProposal(workspace, paperId, proposalId) {
...
//# sourceMappingURL=paper-evolution.js.map