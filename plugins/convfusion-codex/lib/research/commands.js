 ...
 */
function isMethodsExportArg(arg) {
...
function sessionWorkspaceOf(invocation) {
 ...
 */
function openProject(workspace, topic) {
 ...
 */
function describeProject(workspace) {
    ...
    lines.push('');
    lines.push(`Skills: ${skills.counts.total} (system ${skills.counts.system} · user ${skills.counts.user} · derived ${skills.counts.derived})`);
 ...
 * "命令到底有没有把任务交出去"。
 */
function followup(ctx, agent, text, kind) {
 ...
 */
function buildKickoffText(workspace, topic, root = 'workspace/') {
    void workspace;
    return [
        `开始这项研究：${topic}`,
        '',
 ...
 */
function buildContinueText(workspace, intent, root = '') {
    const project = loadProjectFile(workspace);
    ...
    if (project) {
        ...
        if (project.goal)
            lines.push(`- Goal: ${project.goal}`);
        if (project.questions?.length)
            lines.push(`- Open questions: ${project.questions.join('; ')}`);
    }
    if (plans.length > 0) {
        lines.push('- Existing plans (Markdown assets you can read and refine):');
        for (const p of plans)
            lines.push(`  - \`${root}${p.path}\` [${p.status ?? 'draft'}] — ${p.title}`);
    }
    lines.push('');
    lines.push(`Read the research workspace as needed (research root \`${root || '.'}\`, relative to the session workspace: ` +
 ...
 */
export function defineResearchCommand(ctx, resolveCurrentWorkspace, customizationStore) {
