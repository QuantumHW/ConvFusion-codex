...
export function parsePlanFrontmatter(source) {
...
/** 去掉 frontmatter。 */
export function stripPlanFrontmatter(source) {
 ...
 */
export function parsePlanSections(body) {
    const parsed = parseSections(body);
    return { title: parsed.title?.replace(/^Plan:\s*/i, '') ?? null, sections: parsed.sections };
}
function normalizeStatus(raw) {
    const v = (raw ?? '').trim().toLowerCase();
    return PLAN_STATUSES.includes(v) ? v : 'draft';
}
/** 解析一个 Plan Markdown 文件（不存在/损坏 → null）。 */
export function parsePlanDocument(absPath, relPath) {
    let source;
    try {
...
export function serializePlanDocument(input) {
    ...
    if (input.sourceSkill)
        front.push(`source_skill: ${input.sourceSkill}`);
    if (input.sourceSkillVersion)
        front.push(`source_skill_version: ${input.sourceSkillVersion}`);
    if (input.paper)
        front.push(`paper: ${input.paper}`);
    if (input.researchContext)
        front.push(`research_context: ${input.researchContext}`);
    if (input.reviewPolicy)
        front.push(`review_policy: ${input.reviewPolicy}`);
    if (input.createdAt)
        front.push(`created_at: ${input.createdAt}`);
 ...
 * ════════════════════════════════════════════════════════════════════════ */
/** 按标题取章节（宽松匹配；找不到 → `''`）。 */
export function planSection(doc, ...names) {
...
/** 列出全部 Plan（最近更新在前）。 */
export function listPlanDocuments(workspace) {
...
/** 读一个 Plan。 */
export function readPlan(workspace, id) {
    return listPlanDocuments(workspace).find((p) => p.id === id);
}
/** 纯函数过滤（便于测试与复用）。 */
export function filterPlans(docs, query) {
    let out = [...docs];
    ...
    if (query.type)
        out = out.filter((d) => d.type === query.type);
    if (query.search) {
 ...
 */
export function isLegacyStylePlanId(id) {
    if (/^step\d+$/i.test(id))
        return true;
    if (/^module\d+/i.test(id))
        return true;
    if (/(^|-)step(-|$)/i.test(id))
        return true;
    if (/^plan-\d+$/i.test(id))
