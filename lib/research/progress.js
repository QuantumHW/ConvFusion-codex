...
import { assessAdvance } from './advance.js';
import { MATURITY_DIMENSIONS } from './research-data.js';
 ...
 */
export function captureProgress(workspace, skillContent, advanceOptions = {}) {
...
export function diffProgress(before, after) {
    ...
    const keys = Object.keys(after.counts);
    const countChanges = keys
        .filter((k) => before.counts[k] !== after.counts[k])
        .map((k) => ({ key: k, from: before.counts[k], to: after.counts[k] }));
    return {
        before,
        after,
...
const BAR_WIDTH = 20;
/** 一条进度条（`█` 填充 + `░` 空白）。 */
export function renderBar(scale, width = BAR_WIDTH) {
 ...
 */
export function renderProgressNotice(diff, advance) {
    ...
    const summary = `📊 研究进展 · 成熟度折算 ${pct(ovBefore)} → ${pct(ovAfter)}` +
    ...
    lines.push('## 📊 本轮研究进展');
    lines.push('');
    lines.push(`**成熟度（等级折算，非测量值）**  ${pct(ovBefore)} → ${pct(ovAfter)}` +
    ...
    if (unsupported > 0)
        lines.push(`- ${unsupported} 条主张尚缺支撑证据`);
    const noArtifact = diff.after.counts.evidence - diff.after.counts.evidenceWithArtifact;
    if (noArtifact > 0)
        lines.push(`- ${noArtifact} 条证据缺原始产物引用（provenance 不完整）`);
    if (diff.after.counts.openQuestions > 0)
        lines.push(`- ${diff.after.counts.openQuestions} 个开放问题待解`);
    lines.push('');
    if (advance) {
        lines.push('**推进判定**');
        lines.push('');
        const label = advance.clarity === 'clear'
            ? '方向明确 → 可直接推进'
        ...
        if (advance.nextStep)
            lines.push(`- 下一步：${advance.nextStep}`);
        if (advance.needsUserDecision)
            lines.push(`- 待你决定：${advance.needsUserDecision}`);
        lines.push('');
    }
 ...
 */
export function researchGaps(snapshot) {
    const out = [];
    out.push(snapshot.stage
        ? `科研过程当前阶段：${snapshot.stage.label}（尚未落地）`
        : '科研过程各阶段均已有落地资产');
    const unsupported = snapshot.counts.claims - snapshot.counts.claimsSupported;
    if (unsupported > 0)
        out.push(`${unsupported} 条主张尚缺支撑证据`);
    const noArtifact = snapshot.counts.evidence - snapshot.counts.evidenceWithArtifact;
    if (noArtifact > 0)
        out.push(`${noArtifact} 条证据缺原始产物引用（provenance 不完整）`);
    if (snapshot.counts.openQuestions > 0)
        out.push(`${snapshot.counts.openQuestions} 个开放问题待解`);
    return out;
}
/** 由差异 + 快照构造界面用的回合报告。 */
export function buildTurnReport(diff, turn, advance, at = new Date()) {
    const moved = diff.changed;
    const ovDelta = Math.round((diff.overallAfter - diff.overallBefore) * 100);
    const summary = `研究进展 · 成熟度折算 ${pct(diff.overallBefore)} → ${pct(diff.overallAfter)}` +
        (ovDelta !== 0 ? ` (${ovDelta > 0 ? '+' : ''}${ovDelta})` : '') +
