...
import { OUTPUT_AUX_DIRS, OUTPUT_DIRS, OUTPUT_ID_PREFIX, OUTPUT_MAIN_FILE, OUTPUT_STATUSES, OUTPUT_TYPES, OUTPUT_VERSIONS_DIR, OUTPUTS_DIR, } from './output-data.js';
import { getOutputProfile } from './output-profiles.js';
import { findSection, parseFrontmatter, parseIdList, parseSections, renderFrontmatter, renderSections, stripFrontmatter, } from './markdown.js';
/* ════════════════════════════════════════════════════════════════════════
 * 路径
 * ════════════════════════════════════════════════════════════════════════ */
/**
 ...
 * 而不是把它们压成同名文件。
 */
export function outputDir(workspace, type, id) {
    if (type === 'paper')
        return join(workspace, 'papers', id);
    return join(workspace, OUTPUTS_DIR, OUTPUT_DIRS[type], id);
}
/** 相对 workspace 的成果目录（如 `outputs/patents/patent-001`）。 */
export function outputRelDir(type, id) {
    if (type === 'paper')
        return `papers/${id}`;
    return `${OUTPUTS_DIR}/${OUTPUT_DIRS[type]}/${id}`;
}
/** 一个类型目录（绝对；= 该类型下所有成果目录的父目录）。 */
export function outputTypeDir(workspace, type) {
    if (type === 'paper')
        return join(workspace, 'papers');
    return join(workspace, OUTPUTS_DIR, OUTPUT_DIRS[type]);
}
/** 相对 workspace 的类型目录。 */
export function outputTypeRelDir(type) {
    if (type === 'paper')
        return 'papers';
...
function normalizeStatus(raw) {
    ...
    return OUTPUT_STATUSES.includes(v) ? v : 'draft';
}
function normalizeType(raw) {
    const v = (raw ?? '').trim().toLowerCase();
    return OUTPUT_TYPES.includes(v) ? v : 'technical-report';
}
/**
 ...
 */
function deriveOutputId(absPath, frontmatterId) {
    ...
    const typeDirs = new Set(Object.values(OUTPUT_DIRS));
    if (parent && !typeDirs.has(parent))
        return parent;
    return file.replace(/\.md$/, '');
}
/** 解析一个 Output Artifact。 */
export function parseOutputArtifact(absPath, relPath, typeHint) {
...
function profileHeading(type, title) {
    const prefix = {
        paper: 'Paper',
        patent: 'Patent',
 ...
 */
export function listOutputs(workspace, type) {
    if (type === 'paper')
        return [];
    const out = [];
    let dirs;
 ...
 */
export function listAllOutputs(workspace) {
    return OUTPUT_TYPES.filter((t) => t !== 'paper').flatMap((t) => listOutputs(workspace, t));
}
/** 按 id 找 Output（跨类型）。 */
export function readOutput(workspace, id) {
    return listAllOutputs(workspace).find((o) => o.id === id);
}
/** 下一个 id（如 `patent-001`）。 */
export function nextOutputId(workspace, type) {
...
export function isOutputWriteError(v) {
 ...
 * 不需要为每种类型写一段代码。
 */
export function outputSectionsTemplate(profile, context = {}) {
    return profile.structure.map((title) => ({
        title,
        body: title.toLowerCase().includes('claim') && profile.type === 'patent'
 ...
 */
export function createOutput(workspace, input) {
    const title = input.title.trim();
    if (!title)
        return { error: 'Output 需要一个标题。' };
    const id = (input.id ?? nextOutputId(workspace, input.type)).trim();
    if (!/^[a-z0-9][a-z0-9-]*$/.test(id))
        return { error: `非法 Output id：${id}（kebab-case）` };
    const file = outputPath(workspace, input.type, id);
    if (existsSync(file))
        return { error: `Output \`${id}\` 已存在。` };
    ...
    for (const sub of OUTPUT_AUX_DIRS[input.type])
        mkdirSync(join(dir, sub), { recursive: true });
    writeFileSync(file, serializeOutputArtifact({
        id,
        title,
        type: input.type,
        status: 'draft',
 ...
 */
export function writePatentClaimsFile(workspace, id) {
    const doc = readOutput(workspace, id);
    if (!doc || doc.type !== 'patent')
        return undefined;
    const lines = [
        '# Patent Claims (draft)',
        '',
 ...
 * （表现为"保存成功但内容没变"）。
 */
export function saveOutputBody(workspace, id, body) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return { error: `找不到 Output \`${id}\`。` };
    const sections = parseSections(stripFrontmatter(body)).sections;
    return writeOutput(workspace, doc, { sections });
}
/** 替换单个章节。 */
export function updateOutputSection(workspace, id, section, body) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return { error: `找不到 Output \`${id}\`。` };
...
function touchOutput(workspace, doc) {
    void workspace;
    return writeOutput(workspace, doc, {});
}
function writeOutput(workspace, doc, patch) {
    void workspace;
    writeFileSync(doc.path, serializeOutputArtifact({
        id: doc.id,
...
export function bumpOutputVersion(version, kind = 'minor') {
...
export function archiveOutputVersion(workspace, id, options = {}) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return undefined;
...
/** 列出历史版本。 */
export function listOutputVersions(workspace, id) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return [];
    const dir = join(outputDir(workspace, doc.type, id), 'history');
...
/** 读某个历史版本。 */
export function readOutputVersion(workspace, id, version) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return undefined;
    return (readTextOrNull(join(outputDir(workspace, doc.type, id), 'history', `v${version.replace(/^v/i, '')}.md`)) ?? undefined);
}
/* ════════════════════════════════════════════════════════════════════════
 * Review（Task 7 / §19）
 * ════════════════════════════════════════════════════════════════════════ */
/** 处置记录文件（`<成果目录>/history/reviews.json`）。 */
function reviewsPath(workspace, id) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return join(workspace, OUTPUT_VERSIONS_DIR, id, 'reviews.json');
    return join(outputDir(workspace, doc.type, id), 'history', 'reviews.json');
}
/** 读处置记录（who / when / based on what，§19）。 */
export function readOutputReviews(workspace, id) {
...
function appendReview(workspace, id, record) {
    const doc = readOutput(workspace, id);
    const dir = doc
 ...
 */
export function reviewOutput(workspace, id, action, options = {}) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return { error: `找不到 Output \`${id}\`。` };
    ...
    const nextStatus = action === 'approved' ? 'approved' : action === 'reviewed' ? 'reviewed' : action === 'archived' ? 'archived' : doc.status;
...
export function archiveOutput(workspace, id, note) {
 ...
 */
export function deleteOutput(workspace, id, options = {}) {
    if (!options.confirm) {
 ...
 * 但只有规则能给出同样的结论两次。
 */
export function checkOutputQuality(workspace, id) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return { error: `找不到 Output \`${id}\`。` };
 ...
 */
export function traceOutputProvenance(workspace, id) {
    const doc = readOutput(workspace, id);
    if (!doc)
        return undefined;
    const hops = [{ kind: 'output', ref: doc.id, relation: `${doc.type} v${doc.version}` }];
    if (doc.skill)
        hops.push({ kind: 'skill', ref: doc.skill, relation: 'produced by skill' });
    if (doc.plan)
        hops.push({ kind: 'plan', ref: doc.plan, relation: 'via plan' });
    if (doc.harnessSession)
        hops.push({ kind: 'session', ref: doc.harnessSession, relation: 'executed in session' });
    ...
    if (doc.source.paper)
        hops.push({ kind: 'source', ref: doc.source.paper, relation: 'from paper' });
    for (const p of doc.source.plans)
        hops.push({ kind: 'plan', ref: p, relation: 'from plan' });
    for (const c of doc.source.claims)
        hops.push({ kind: 'claim', ref: c, relation: 'uses claim' });
    for (const e of doc.source.evidence)
        hops.push({ kind: 'evidence', ref: e, relation: 'uses evidence' });
 ...
 */
export function writeOutputProvenance(workspace, id) {
 ...
 */
export function buildOutputDependencyMap(workspace) {
    const byKey = new Map();
    const add = (kind, key, doc) => {
    ...
    for (const doc of listAllOutputs(workspace)) {
        ...
        for (const p of doc.source.plans)
            add('plan', p, doc);
        if (doc.source.paper)
            add('paper', doc.source.paper, doc);
        if (doc.source.researchState)
            add('research-state', `v${doc.source.researchState}`, doc);
    }
 ...
 */
export function analyzeOutputImpact(workspace, changed, kind) {
    const map = buildOutputDependencyMap(workspace);
    const key = changed.trim();
    const matches = map.filter((d) => (kind ? d.kind === kind : true) && (d.dependsOn === key || d.dependsOn === key.toUpperCase()));
    return matches.map((d) => ({
        changed: d.dependsOn,
        kind: d.kind,
        affected: d.outputs.map((o) => ({
            ...o,
            recommendation: d.kind === 'claim'
...
export function outputImpactSummary(workspace) {
    const all = listAllOutputs(workspace);
    const byType = {};
    for (const t of OUTPUT_TYPES)
        byType[t] = all.filter((o) => o.type === t).length;
    return {
