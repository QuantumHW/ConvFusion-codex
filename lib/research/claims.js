...
import { CLAIMS_DIR, CLAIM_STATUSES, DECISIONS_DIR, makeResearchId, parseResearchId, } from './research-data.js';
import { findSection, parseFrontmatter, parseIdList, parseSections, renderFrontmatter, renderSections, stripFrontmatter, deriveShortName, } from './markdown.js';
import { listEvidence } from './evidence.js';
export function isIdWriteError(v) {
...
function nextIdIn(workspace, dir, kind) {
 ...
 * Claim（§11 / §40-C）
 * ════════════════════════════════════════════════════════════════════════ */
function normalizeClaimStatus(raw) {
    const v = (raw ?? '').trim().toLowerCase();
    return CLAIM_STATUSES.includes(v) ? v : 'unverified';
}
/** 解析一个 Claim Markdown。 */
export function parseClaimDocument(absPath, relPath) {
    let source;
    try {
...
/** 列出全部 Claim。 */
export function listClaims(workspace) {
...
/** 读一个 Claim。 */
export function readClaim(workspace, id) {
    const norm = id.trim().toUpperCase();
    return listClaims(workspace).find((c) => c.id === norm);
}
/** 创建 Claim（§40-C）。 */
export function createClaim(workspace, input) {
    const statement = input.statement.trim();
    if (!statement)
        return { error: 'Claim 内容不能为空。' };
    const id = (input.id ?? nextIdIn(workspace, CLAIMS_DIR, 'claim')).trim().toUpperCase();
    const parsed = parseResearchId(id);
    if (!parsed || parsed.kind !== 'claim')
        return { error: `非法 Claim id：${id}（应为 C001 形式）` };
    const dir = join(workspace, CLAIMS_DIR);
    mkdirSync(dir, { recursive: true });
    const file = join(dir, `${id}.md`);
    if (existsSync(file))
        return { error: `Claim \`${id}\` 已存在。` };
    const now = new Date().toISOString();
    writeFileSync(file, serializeClaimDocument({
        id,
...
function writeClaim(workspace, doc, patch) {
    writeFileSync(doc.path, serializeClaimDocument({
        id: doc.id,
...
export function setClaimStatus(workspace, id, status) {
    const doc = readClaim(workspace, id);
    if (!doc)
        return { error: `找不到 Claim \`${id}\`。` };
 ...
 *   - 无有效证据 → 保持 `unverified`
 */
export function reconcileClaimEvidence(workspace, id) {
    const doc = readClaim(workspace, id);
    if (!doc)
        return { error: `找不到 Claim \`${id}\`。` };
    ...
    if (allRejected)
        status = 'rejected';
    else if (hasSupported && against.length === 0)
 ...
 */
export function checkClaimEvidenceConsistency(workspace, claimId) {
...
export function deleteClaim(workspace, id) {
    const doc = readClaim(workspace, id);
    if (!doc)
        return { error: `找不到 Claim \`${id}\`。` };
    try {
...
export function parseDecisionDocument(absPath, relPath) {
    let source;
    try {
        source = readFileSync(absPath, 'utf8');
    }
...
/** 读一个 Decision。 */
export function readDecision(workspace, id) {
    const norm = id.trim().toUpperCase();
 ...
 */
export function createDecision(workspace, input) {
    const decision = input.decision.trim();
    if (!decision)
        return { error: 'Decision 内容不能为空。' };
    ...
    if (!parsed || parsed.kind !== 'decision')
        return { error: `非法 Decision id：${id}（应为 D001 形式）` };
    const dir = join(workspace, DECISIONS_DIR);
    mkdirSync(dir, { recursive: true });
    const file = join(dir, `${id}.md`);
    if (existsSync(file))
        return { error: `Decision \`${id}\` 已存在。` };
    const now = new Date().toISOString();
    writeFileSync(file, serializeDecisionDocument({
        id,
        ...(input.name?.trim() ? { name: input.name.trim() } : {}),
        decision,
        reason: input.reason ?? '',
...
export function setDecisionStatus(workspace, id, status) {
    const doc = readDecision(workspace, id);
    if (!doc)
        return { error: `找不到 Decision \`${id}\`。` };
    const sections = doc.sections.map((s) => (s.title === 'Status' ? { title: 'Status', body: status } : s));
    writeFileSync(doc.path, serializeDecisionDocument({
        id: doc.id,
...
/** 删除 Decision。 */
export function deleteDecision(workspace, id) {
    const doc = readDecision(workspace, id);
    if (!doc)
        return { error: `找不到 Decision \`${id}\`。` };
    try {
 ...
 */
export function traceEvidenceProvenance(workspace, evidenceId) {
...
export function traceClaimProvenance(workspace, claimId) {
    const claim = readClaim(workspace, claimId);
    if (!claim)
        return undefined;
    const all = listEvidence(workspace);
    ...
    const hops = [{ kind: 'claim', ref: claim.id, relation: 'claim' }];
    for (const e of supporting)
        hops.push({ kind: 'evidence', ref: e.id, relation: 'supports' });
    for (const e of against)
        hops.push({ kind: 'evidence', ref: e.id, relation: 'contradicts' });
