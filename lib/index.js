...
import { Config, OPENALEX_API_KEY_ENV, resolveConfig, resolveCustomizationPath, } from './config.js';
...
import { mountEventBridge } from './research/runtime-events.js';
...
export function apply(ctx, rawConfig = {}) {
    ...
    const customizationStore = createFileCustomizationStore(() => resolveCustomizationPath(currentConfig()));
    ctx.logger?.info(`[convfusion] user customizations: ${customizationStore.description}` +
    ...
    const resolveCurrentSessionWorkspace = () => resolveWorkspace(currentCwd, undefined);
    const resolveCurrentWorkspace = () => researchWorkspaceOf(resolveCurrentSessionWorkspace());
    ...
    const resolveWorkspaceForAgent = (agent) => {
        const sessionWs = typeof agent?.id === 'string' && agent.id.trim() ? resolveSessionWorkspace(ctx, agent.id) : undefined;
        if (sessionWs)
            return researchWorkspaceOf(sessionWs);
        return resolveCurrentWorkspace();
    };
    // ── 1. Research Context（核心：动态上下文注入）─────────────────────────
    // 过程定义是一个**可定制能力**：读它的"生效正文"必须经过用户覆盖层合成。
    const skillContent = (id) => effectiveSkillContentById(id, customizationStore);
    ...
    // 不新增自定义事件类型，也不唤醒新一轮。
    ctx.effect(() => mountProgressBridge(ctx, resolveCurrentWorkspace, skillContent, {
        // 每次求值：用户在设置里改完立即生效
    ...
    ctx.effect(() => {
        ...
        { fetchImpl: globalThis.fetch }).map((tool) => ctx.tools.register(tool));
        return () => {
            for (const d of disposers) {
                try {
                    d();
    ...
    ctx.inject(['webServer', 'connection'], (cctx) => {
        const webServer = cctx.get('webServer');
        if (!webServer || typeof webServer.register !== 'function') {
            ctx.logger?.warn(`[convfusion] webServer 不可用；${SETTINGS_ROUTE_PREFIX} 路由未注册（设置页将无法读取）`);
            return;
        }
        ...
        const handler = createSettingsRouteHandler({
            ...
            ...(connection && typeof connection.requestRejection === 'function'
                ? {
                    reject: (req) => connection.requestRejection({
                        headers: req.headers,
        ...
        try {
            ...
            ctx.logger?.error(`[convfusion] ${SETTINGS_ROUTE_PREFIX} 路由注册失败：${e instanceof Error ? e.message : String(e)}`);
            return;
        }
    ...
    ctx.logger?.info('[convfusion] Stage 1 runtime mounted — research context + skill provider + event bridge' +
