...
/** Paper 目录（绝对）。 */
export function paperDir(workspace: string, paperId: string = DEFAULT_PAPER_ID): string {
  return join(workspace, PAPERS_DIR, paperId)
}

/** 相对 workspace 的 Paper 目录。 */
export function paperRelDir(paperId: string = DEFAULT_PAPER_ID): string {
  return `${PAPERS_DIR}/${paperId}`
}

function filePath(workspace: string, paperId: string, name: string): string {
...
function normalizeStatus(raw: string | undefined): PaperStatus {
  const v = (raw ?? '').trim().toLowerCase()
  return (PAPER_STATUSES as readonly string[]).includes(v) ? (v as PaperStatus) : 'evolving'
}

function parseMetadata(source: string | null, fallbackId: string): PaperMetadata {
...
export function serializeMetadata(meta: PaperMetadata): string {
...
/** 列出全部 Paper id。 */
export function listPaperIds(workspace: string): string[] {
  try {
 ...
 */
export function readPaper(workspace: string, paperId: string = DEFAULT_PAPER_ID): PaperDocument | null {
  ...
  if (manuscriptSource === null && metadataSource === null) return null
...
/* ════════════════════════════════════════════════════════════════════════
 * 写（Task 1 create / update）
 * ════════════════════════════════════════════════════════════════════════ */

export interface PaperWriteError {
  error: string
}
export function isPaperWriteError(v: unknown): v is PaperWriteError {
 ...
 * 而不是返回一个需要额外判空的 `null`。
 */
function mustRead(workspace: string, paperId: string, action: string): PaperDocument | PaperWriteError {
  const paper = readPaper(workspace, paperId)
  return paper ?? { error: `找不到 Paper \`${paperId}\`，无法${action}。` }
}

/** 最小 Paper 正文骨架（§7 的推荐章节；**可裁剪**）。 */
export function manuscriptTemplate(title?: string): string {
...
export function createPaper(
  ...
  writeFileSync(
    filePath(workspace, paperId, PAPER_FILES.manuscript),
    input.manuscript ?? manuscriptTemplate(input.title),
    'utf8',
  )
  writeFileSync(
    filePath(workspace, paperId, PAPER_FILES.metadata),
    serializeMetadata({
...
export function updatePaperMetadata(
  workspace: string,
  paperId: string,
  patch: Partial<Omit<PaperMetadata, 'id'>>,
): PaperDocument | PaperWriteError {
  const paper = readPaper(workspace, paperId)
  if (!paper) return { error: `找不到 Paper \`${paperId}\`。` }
  const meta: PaperMetadata = { ...paper.metadata, ...patch, id: paper.metadata.id }
...
/** 整体保存正文（编辑器路径）。 */
export function saveManuscript(workspace: string, paperId: string, manuscript: string): PaperDocument | PaperWriteError {
  const paper = readPaper(workspace, paperId)
  if (!paper) return { error: `找不到 Paper \`${paperId}\`。` }
...
export function updatePaperSection(
  workspace: string,
  paperId: string,
  section: string,
  body: string,
  ...
  else sections.push({ title: normalizeSectionName(section), body })
...
function touchMetadata(workspace: string, paperId: string): PaperDocument | PaperWriteError {
  const paper = readPaper(workspace, paperId)
  if (!paper) return { error: `找不到 Paper \`${paperId}\`。` }
  writeFileSync(
    filePath(workspace, paperId, PAPER_FILES.metadata),
    serializeMetadata({ ...paper.metadata, updatedAt: new Date().toISOString() }),
    'utf8',
  )
...
/** 递增论文版本：`0.4` → `0.5`。 */
export function bumpPaperVersion(version: string, kind: 'minor' | 'major' = 'minor'): string {
...
export function archivePaperVersion(
  workspace: string,
  paperId: string,
  options: { reason?: string; version?: string } = {},
): PaperVersionEntry | undefined {
  const paper = readPaper(workspace, paperId)
  if (!paper || !paper.present.manuscript) return undefined
...
/** 列出历史版本。 */
export function listPaperVersions(workspace: string, paperId: string): PaperVersionEntry[] {
  const dir = filePath(workspace, paperId, PAPER_FILES.historyDir)
  let files: string[]
  try {
...
export function readPaperVersion(workspace: string, paperId: string, version: string): string | undefined {
  return readTextOrNull(
    join(filePath(workspace, paperId, PAPER_FILES.historyDir), `v${version.replace(/^v/i, '')}.md`),
  ) ?? undefined
}
 ...
 */
export function restorePaperVersion(
  workspace: string,
  paperId: string,
  version: string,
  reason = `restore from v${version}`,
): PaperDocument | PaperWriteError {
  const historical = readPaperVersion(workspace, paperId, version)
  if (historical === undefined) return { error: `找不到历史版本 v${version}。` }
  const paper = readPaper(workspace, paperId)
  if (!paper) return { error: `找不到 Paper \`${paperId}\`。` }
  ...
  writeFileSync(
    filePath(workspace, paperId, PAPER_FILES.manuscript),
    historical.replace(ARCHIVE_HEADER_RE, ''),
    'utf8',
  )
...
export function diffManuscript(
  workspace: string,
  paperId: string,
  other: { version?: string; text?: string },
): { added: string[]; removed: string[]; changedLines: number } | PaperWriteError {
  const paper = readPaper(workspace, paperId)
  if (!paper) return { error: `找不到 Paper \`${paperId}\`。` }
  const target = other.text ?? (other.version ? readPaperVersion(workspace, paperId, other.version) : undefined)
  if (target === undefined) return { error: '需要提供 version 或 text 作为对比目标。' }
 ...
 * "这句话对应哪个 Claim"（§8 的意图）。
 */
export function readClaimMap(workspace: string, paperId: string = DEFAULT_PAPER_ID): PaperClaimEntry[] {
  const paper = readPaper(workspace, paperId)
  if (!paper) return []
  ...
  if (source) {
    ...
    const entries: PaperClaimEntry[] = []
    for (const s of sections) {
      if (!/^C\d{1,4}$/.test(s.title.trim())) continue
      entries.push({
        id: s.title.trim(),
        ...
        sections: (findSection([s], 'Paper Sections').match(/^[-*]\s*(.+)$/gm) ?? []).map((l) =>
          l.replace(/^[-*]\s*/, '').trim(),
        ),
      })
    }
...
function mergeWithResearchClaims(
  entries: readonly PaperClaimEntry[],
  workspace: string,
  options: { requireInManuscript?: boolean; derived?: readonly PaperClaimEntry[] } = {},
): PaperClaimEntry[] {
  ...
  const byId = new Map<string, PaperClaimEntry>()
  ...
  for (const [id, c] of authoritative) {
    ...
    if (!existing && requireInManuscript) continue
    byId.set(id, {
      id,
...
export function deriveClaimMapFromManuscript(paper: PaperDocument): PaperClaimEntry[] {
  const byClaim = new Map<string, PaperClaimEntry>()
 ...
 */
export function buildEvidenceMap(workspace: string, paperId: string = DEFAULT_PAPER_ID): PaperEvidenceEntry[] {
  const paper = readPaper(workspace, paperId)
  if (!paper) return []
  const claims = readClaimMap(workspace, paperId)
  const byEvidence = new Map<string, PaperEvidenceEntry>()

  // 1) 从 Claim Map 汇总 supports / contradicts
  for (const c of claims) {
    for (const e of c.evidence) {
      const entry = byEvidence.get(e) ?? { id: e, supports: [], contradicts: [], sections: [] }
      if (!entry.supports.includes(c.id)) entry.supports.push(c.id)
      entry.sections.push(...c.sections)
      byEvidence.set(e, entry)
    }
    for (const e of c.contradictions) {
      const entry = byEvidence.get(e) ?? { id: e, supports: [], contradicts: [], sections: [] }
      if (!entry.contradicts.includes(c.id)) entry.contradicts.push(c.id)
      byEvidence.set(e, entry)
    }
  }
  ...
  for (const section of paper.sections) {
    ...
    for (const id of ids) {
      ...
      if (!entry.sections.includes(section.title)) entry.sections.push(section.title)
...
export function writeEvidenceMap(
  workspace: string,
  paperId: string,
  entries: readonly PaperEvidenceEntry[],
): string {
...
function evolutionIndexPath(workspace: string, paperId: string): string {
  return filePath(workspace, paperId, EVOLUTION_INDEX)
}

/** 读演化事件索引。 */
export function listEvolutionEvents(workspace: string, paperId: string = DEFAULT_PAPER_ID): EvolutionEvent[] {
  const raw = readTextOrNull(evolutionIndexPath(workspace, paperId))
  if (!raw) return []
  try {
    const parsed = JSON.parse(raw) as EvolutionEvent[]
...
export function recordEvolutionEvent(
  workspace: string,
  paperId: string,
  event: Omit<EvolutionEvent, 'id' | 'timestamp'> & { id?: string; timestamp?: string },
): EvolutionEvent {
  const events = listEvolutionEvents(workspace, paperId)
  const id = event.id ?? `EV${String(events.length + 1).padStart(3, '0')}`
  const full: EvolutionEvent = { ...event, id, timestamp: event.timestamp ?? new Date().toISOString() }
...
export function archivePaper(workspace: string, paperId: string): PaperDocument | PaperWriteError {
  return updatePaperMetadata(workspace, paperId, { status: 'archived' })
}

/** 从归档恢复为演化中。 */
export function restoreArchivedPaper(workspace: string, paperId: string): PaperDocument | PaperWriteError {
  return updatePaperMetadata(workspace, paperId, { status: 'evolving' })
}

/** 冻结（暂时不再演化）—— 与归档不同，仍视为当前论文。 */
export function freezePaper(workspace: string, paperId: string): PaperDocument | PaperWriteError {
...
): { ok: true; id: string } | PaperWriteError {
  if (!options.confirm) {
    return { error: `删除 Paper \`${paperId}\` 会丢失其演化历史。请先考虑 archive；确需删除请显式确认。` }
  }
  const dir = paperDir(workspace, paperId)
  if (!existsSync(dir)) return { error: `找不到 Paper \`${paperId}\`。` }
  try {
