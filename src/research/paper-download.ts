...
/** 从任意 URL 提取 arXiv id。 */
export function extractArxivId(url: string | undefined): string | undefined {
 ...
 */
export function extractDoi(doi: string | undefined): string | undefined {
  ...
  if (m) return m[1]
  if (/^10\./.test(trimmed)) return trimmed
  return undefined
}
 ...
 */
function aclToPdf(url: string): string | undefined {
 ...
 * 去重，保持顺序。纯函数：不联网。
 */
export function resolveDownloadCandidates(links: PaperLinks): DownloadCandidate[] {
  const out: DownloadCandidate[] = []
  const seen = new Set<string>()

  const push = (url: string | undefined, source: string): void => {
    if (!url) return
    const u = url.trim()
  ...
  if (doi) push(`https://doi.org/${doi}`, 'doi')
 ...
 * - 占位：`<seq>_<slug>.txt`
 */
export function buildFilename(seq: number, slug: string, ext: 'pdf' | 'html' | 'txt'): string {
...
export function readManifest(workspace: string): FulltextManifest {
  const abs = join(workspace, FULLTEXT_MANIFEST)
  try {
    const raw = readFileSync(abs, 'utf8')
    const parsed = JSON.parse(raw) as Partial<FulltextManifest>
    return {
      ...
      entries: Array.isArray(parsed.entries) ? (parsed.entries as FulltextManifestEntry[]) : [],
    }
  } catch {
...
export function writeManifest(workspace: string, manifest: FulltextManifest): void {
...
export function appendManifestEntry(
  workspace: string,
  entry: Omit<FulltextManifestEntry, 'seq'>,
): { entry: FulltextManifestEntry; manifest: FulltextManifest } {
  const manifest = readManifest(workspace)
  const seq = manifest.nextSeq
  const full: FulltextManifestEntry = { ...entry, seq }
 ...
 */
export async function downloadFromCandidates(
  candidates: DownloadCandidate[],
  deps: PaperDownloadDeps,
): Promise<DownloadOutcome> {
  const doFetch = deps.fetchImpl ?? (globalThis.fetch as unknown as DownloadFetchLike)
