...
function gapsPath(workspace: string, paperId: string): string {
 ...
 */
export function listPaperGaps(workspace: string, paperId: string): PaperGap[] {
  let source: string
  try {
  ...
  // 找出所有 `## G00N` 标题行
  const heads: Array<{ index: number; id: string }> = []
  ...
  for (let h = 0; h < heads.length; h++) {
    ...
    out.push({
      id: heads[h].id,
      type: (field('Type') || 'experimental-validation') as PaperGapType,
      ...
      priority: (field('Priority') || 'medium') as GapPriority,
      detectedBy: (field('Detected By') || 'rule') as PaperGap['detectedBy'],
...
export function serializePaperGaps(gaps: readonly PaperGap[]): string {
  const parts = [
    '# Research Gaps',
    '',
  ...
  if (gaps.length === 0) parts.push('<!-- 规则检查未发现缺口。 -->', '')
  for (const g of gaps) {
    parts.push(`## ${g.id}`, '')
    parts.push(`**Type**: ${g.type}`)
    parts.push(`**Description**: ${g.description}`)
    if (g.relatedClaim) parts.push(`**Related Claim**: ${g.relatedClaim}`)
    if (g.relatedSection) parts.push(`**Related Section**: ${g.relatedSection}`)
    if (g.suggestedSkill) parts.push(`**Suggested Capability**: ${g.suggestedSkill}`)
    parts.push(`**Priority**: ${g.priority}`)
    parts.push(`**Detected By**: ${g.detectedBy}`)
    parts.push(`**Created At**: ${g.createdAt}`)
    if (g.resolved) parts.push(`**Resolved**: ${g.resolved.at}`, `**Resolved By**: ${g.resolved.by}`)
    parts.push('')
  }
  return parts.join('\n').trim() + '\n'
}
...
/** 写回 Gap 列表。 */
export function writePaperGaps(workspace: string, paperId: string, gaps: readonly PaperGap[]): string {
...
/** 下一个 Gap id。 */
export function nextGapId(existing: readonly PaperGap[]): string {
 ...
 */
export const GAP_SKILL_HINTS: Record<PaperGapType, string | undefined> = {
 ...
 */
export function detectPaperGaps(workspace: string, paperId: string): DetectedGap[] {
  ...
  for (const want of REQUIRED_PAPER_SECTIONS) {
    if (!sectionTitles.has(want.toLowerCase())) {
      ...
      const priority: GapPriority = want === 'Abstract' || want === 'Method' ? 'high' : 'medium'
      out.push({
  ...
  const hasExperiment = paper.sections.some(
    (s) => /experiment|result/i.test(s.title) && s.body.replace(/<!--[\s\S]*?-->/g, '').trim().length > 80,
  )
  const mentionsAblation = /ablation/i.test(paper.manuscript)
  const hasAblationEvidence = listEvidence(workspace).some(
    (e) => /ablation/i.test(e.name) || /ablation/i.test(e.type),
  )
  if (hasExperiment && !mentionsAblation && !hasAblationEvidence) {
    out.push({
      type: 'missing-ablation',
      description:
        'The manuscript reports experiments but contains no ablation. There is no evidence isolating which component causes the reported effect.',
  ...
  if (
    ...
    out.push({
      ...
      description:
        'The method/experiment sections do not mention dataset, seeds, environment or hyper-parameters, so the result cannot be reproduced from the paper.',
      priority: 'medium',
    })
  }
 ...
 */
export function mergeDetectedGaps(
  existing: readonly PaperGap[],
  detected: readonly DetectedGap[],
): { gaps: PaperGap[]; added: PaperGap[] } {
...
export function detectAndRecordGaps(
  workspace: string,
  paperId: string,
): { gaps: PaperGap[]; added: PaperGap[]; path: string } {
...
export function recordPaperGap(
  workspace: string,
  paperId: string,
  input: Omit<PaperGap, 'id' | 'createdAt' | 'detectedBy'> & { detectedBy?: PaperGap['detectedBy'] },
): { gap: PaperGap; path: string } {
...
export function resolvePaperGap(
  workspace: string,
  paperId: string,
  gapId: string,
  note?: string,
...
export function prioritizeGaps(gaps: readonly PaperGap[]): PaperGap[] {
...
/* ════════════════════════════════════════════════════════════════════════
 * Capability Recommendation（Task 9 / §15）
 * ════════════════════════════════════════════════════════════════════════ */
...
export function gapSummary(gaps: readonly PaperGap[]): {
  total: number
  open: number
  high: number
...
export function paperExists(workspace: string, paperId: string): boolean {
